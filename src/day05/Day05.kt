...
fun main() {
    ...
    fun part1(input: List<String>): Int {
        val lines = input.map(Line.Companion::parse).filter { line -> line.startingPoint.first == line.endingPoint.first || line.startingPoint.second == line.endingPoint.second }
    ...
    fun part2(input: List<String>): Int {
        val lines = input.map(Line.Companion::parse)
        val ovarlappingPoints = mutableMapOf<Point, Int>()
        lines
    ...
    val input = resourceAsListOfString("src/day05/Day05.txt")
    println(part1(input))
    println(part2(input))
}
data class Point(
